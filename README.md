...
Импорты направлены вниз: `app → widgets → features → entities → shared`. Ограничения между слоями проверяет ESLint. Стили компонентов лежат рядом с UI; базовые стили и подключения — в `app/globals.css`. Глобального менеджера состояния нет: состояние календаря находится в одном React hook, локальное состояние поиска — в компоненте поиска.
