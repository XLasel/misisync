...
Импорты направлены вниз: `app → widgets → features → entities → shared`. Ограничения между слоями проверяет ESLint. CSS Modules (`*.module.css`) лежат рядом с UI: корень компонента — `.root`, дочерние элементы — по смыслу (`.list`, `.heading`). Компонент импортирует свой модуль как `s`; Next.js генерирует уникальные имена с именем модуля и хешем. В `app/globals.css` остаются только reset, базовые стили и переменные темы. Глобального менеджера состояния нет: состояние календаря находится в одном React hook, локальное состояние поиска — в компоненте поиска.
