...
```mermaid
flowchart TD
    UI[React] --> Client[Schedule API client]
...
- Лимит на IP действует в Node.js entrypoint перед публичным Next.js-прокси (`SCHEDULE_RATE_LIMIT_PER_MINUTE`), а не на FastAPI, которому все посетители приходят от Next.js. Хранилище IP ограничено 4096 активными адресами; при заполнении новые адреса временно получают 429. Доверие к внешнему прокси настраивается явно, см. README.
...
## Фронтенд: границы ответственности

- `app/page.tsx` собирает страницу и передаёт московскую дату, чтобы серверный HTML и первая отрисовка React совпадали.
- `widgets/schedule/model/use-calendar.ts` управляет загрузками, выбранной группой, неделей и настройками устройства. Ответ привязан к группе и окну дат; старые ответы игнорируются, запросы отменяются при переключении и размонтировании.
- `widgets/schedule/ui` объединяет поиск, фильтры и список занятий. Нижние слои не импортируют виджеты.
- `features` содержит пользовательские действия; фильтры, навигация и обновление получают данные и обработчики через props.
- `entities/schedule` знает публичный календарный контракт и правила объединения карточек, но не структуру экрана.
- `shared` содержит HTTP, работу с датами и безопасное чтение локальных предпочтений. При запрете localStorage приложение работает без сохранения выбора.

Обновление вынесено под список занятий: время загрузки, кнопка «Обновить» и раскрывающееся пояснение. Нажатие повторяет запрос, сохраняя TTL бэкенда. Единый `--panel-gutter` выравнивает содержимое панели на телефоне, планшете и компьютере.

`server.mjs` читает IP из socket и применяет лимит до передачи запроса Next.js. `app/api/[...path]/route.ts` пропускает только GET четырёх публичных маршрутов; серверный адрес задаётся через `API_BASE` и не попадает в браузер. Cookies и Authorization посетителя не передаются FastAPI. Для production используется тот же entrypoint, что и в dev, и полноценная сборка Next.js с production-зависимостями в Docker.
